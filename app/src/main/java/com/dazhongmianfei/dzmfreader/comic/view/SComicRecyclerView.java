...
public class SComicRecyclerView extends RecyclerView {
   ...
   // private static final String TAG = "999";
    ...
    public SComicRecyclerView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public SComicRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    private void init(AttributeSet attr) {
        ...
        if (attr != null) {
            TypedArray a = getContext()
                    .obtainStyledAttributes(attr, R.styleable.SComicRecyclerView, 0, 0);
            mMinScaleFactor =
                    a.getFloat(R.styleable.SComicRecyclerView_min_scale, DEFAULT_MIN_SCALE_FACTOR);
            mMaxScaleFactor =
                    a.getFloat(R.styleable.SComicRecyclerView_max_scale, DEFAULT_MAX_SCALE_FACTOR);
            mDefaultScaleFactor = a
                    .getFloat(R.styleable.SComicRecyclerView_default_scale, DEFAULT_SCALE_FACTOR);
            mScaleFactor = mDefaultScaleFactor;
            mScaleDuration = a.getInteger(R.styleable.SComicRecyclerView_zoom_duration,
...
/*    @Override
    public boolean onTouchEvent(@NonNull final MotionEvent ev) {
        MyToash.Log("clickScreen 33", "");
        Boolean x = setTouch(ev);
        if (x != null) return x;
//|| retVal
   ...
   /*     if (touchListener != null) {
    ...
    //@Saveflags
    private static int MATRIX_SAVE_FLAG = 0x01;
